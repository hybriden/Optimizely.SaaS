...
async function main() {
  try {
    console.log('🔍 Discovering CMS components and teasers...\n');
    ...
    console.log(`\n📝 Generating ${path.relative(projectRoot, registryPath)}...`);
    fs.writeFileSync(registryPath, registryContent, 'utf8');

    console.log(`\n✨ Successfully generated client registry!`);
    console.log(`   Total registrations: ${components.length + teasers.length}`);
    console.log(`   - Components: ${components.length}`);
    console.log(`   - Teasers: ${teasers.length}\n`);

  } catch (error) {
    console.error(`\n❌ Error generating client registry: ${error.message}`);
