...
export function TeaserCard({
  ...
  const cardClassName = "card relative h-full overflow-hidden block";
    ...
    <>
        {imageUrl ? (
          ...
          <div className={'relative bg-[var(--bg-tertiary)] overflow-hidden flex items-center justify-center border-b border-[var(--border-subtle)] ' + imageHeightClasses[variant]}>
            <svg className="w-12 h-12 text-[var(--neon-cyan)] opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-6 space-y-3">
          {(category || date) && (
            <div className="flex flex-wrap items-center gap-3 text-sm">
              {category && <span className="px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-full bg-[var(--bg-glass)] text-[var(--neon-cyan)] border border-[var(--border-glow)] backdrop-blur-sm">{category}</span>}
              {date && <span className="text-[var(--text-muted)] font-mono">{new Date(date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</span>}
            </div>
          )}
          <h3 className={'font-bold text-[var(--text-primary)] group-hover:text-[var(--neon-cyan)] transition-colors ' + (variant === 'featured' ? 'text-2xl md:text-3xl' : variant === 'compact' ? 'text-lg' : 'text-xl')}>{title}</h3>
          ...
          {children}
          {url && (<div className="flex items-center gap-2 text-[var(--neon-cyan)] font-semibold text-sm group-hover:gap-4 transition-all pt-2"><span>Read more</span><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg></div>)}
        </div>
    </>
  );
